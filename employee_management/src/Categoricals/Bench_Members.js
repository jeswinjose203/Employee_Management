...
const BilledMembers = () => {
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        const response = await axios.get('http://localhost:5001/employee/bench');
  ...
  return (
    <Container>
      ...
      <Title level={1}>Bench Members</Title>
      ...
      {selectedMember && (
        <Modal
          ...
          footer={null}
        >
          <p><strong>EmpCode</strong> {selectedMember.empCode}</p> {/* Use empName */}
